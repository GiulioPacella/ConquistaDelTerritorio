...
/*
 * net_util.h — utility di rete riusabili da server e client:
 *   - buffer di INPUT con framing a righe (gestisce letture parziali/aggregate);
 *   - coda di OUTPUT dinamica (gestisce write parziali ed EAGAIN su socket
 *     non bloccanti);
 *   - creazione del listening socket (server) e connessione (client);
 *   - impostazione della modalità non bloccante.
 */

#include "common.h"
#include <stddef.h>

/* Capacità del buffer grezzo di input per ogni client. Maggiore di MAX_LINE per
   poter accogliere più comandi corti aggregati in un'unica read(). */
